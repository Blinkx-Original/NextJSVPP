...
test('buildCategoryVariants matches Industrial Robots rows', () => {
  ...
  assert.ok(
    variants.normalized.includes('industrial-robots'),
    'includes original slug in normalized variants'
  );

  const normalizedRow = 'Industrial Robots'.trim().toLowerCase();
  assert.ok(
    variants.normalized.includes(normalizedRow),
    'matches normalized category value for display name'
  );

  assert.ok(
    variants.raw.includes('Industrial Robots'),
    'preserves original cased category name for JSON comparisons'
  );

  assert.ok(variants.collapsed.includes('industrialrobots'), 'includes collapsed variant');
});
