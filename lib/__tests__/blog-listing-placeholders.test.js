...
test('blog listing placeholders slugify and load products', async () => {
  ...
  const overrides = {
    ...
    '@/lib/categories': {
      ...
      getPublishedProductsForCategory: async (category) => {
        productCalls.push({ fn: 'getPublishedProductsForCategory', slug: category.slug, name: category.name });
        return {
          products: [
            {
              id: BigInt(7),
  ...
  const modulePath = path.resolve(projectRoot, 'app/b/[slug]/product-listing.ts');
  const helpers = loadTsModule(modulePath, overrides);

  const { extractProductListingPlaceholders, loadCategoryListing } = helpers;
  ...
  assert.equal(placeholder.config.categoryLabel, 'Máquinas CNC');

  const listing = await loadCategoryListing({
    config: { ...placeholder.config, slug: placeholder.config.slug ?? '' },
    pageParam: 1,
    pageKey: 'page',
    requestId: 'test'
  });
  ...
  assert.equal(categoryCall.name, 'Máquinas CNC');
});

test('product listing parser accepts category_slug attribute', () => {
  const overrides = {
    '@/lib/categories': {
      getPublishedProductsForCategory: async () => ({ products: [], totalCount: 0 })
    },
    '@/lib/products': {
      getPublishedProductsBySlugs: async () => []
    }
  ...
  const modulePath = path.resolve(projectRoot, 'app/b/[slug]/product-listing.ts');
  const { extractProductListingPlaceholders } = loadTsModule(modulePath, overrides);

  const html = '<!-- product-listing category_slug="industrial" -->';
  const { placeholders } = extractProductListingPlaceholders(html);

  assert.equal(placeholders.length, 1);
  const placeholder = placeholders[0];
  assert.equal(placeholder.config.type, 'category');
  assert.equal(placeholder.config.slug, 'industrial');
  assert.equal(placeholder.config.categoryLabel, 'industrial');
});
