...
import { getSiteUrl, buildProductCategoryUrl } from '@/lib/urls';
...
export async function GET(): Promise<Response> {
  const path = '/sitemaps/categories.xml';
  ...
  try {
    ...
    const productEntries = entries.filter((entry) => entry.type === 'product');
    const xml = renderUrlsetXml(
      productEntries.map((entry) => ({
        loc: buildProductCategoryUrl(entry.slug, host),
    ...
    console.error('[sitemap-categories] error', error, { requestId });
    return NextResponse.json({ error: 'categories_sitemap_error' }, { status: 500 });
  }
}
